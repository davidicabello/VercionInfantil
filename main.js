...
const EVENT_DATE = new Date('2025-10-24T19:00:00');
...
if (shareWhatsapp) {
  shareWhatsapp.addEventListener('click', function() {
    const url = `https://wa.me/?text=${encodeURIComponent(SHARE_TEXT + ' ' + INVITATION_URL)}`;
    window.open(url, '_blank');
  });
}
if (shareFacebook) {
  shareFacebook.addEventListener('click', function() {
    const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(INVITATION_URL)}`;
    window.open(url, '_blank');
  });
}
if (shareTwitter) {
  shareTwitter.addEventListener('click', function() {
    const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(SHARE_TEXT)}&url=${encodeURIComponent(INVITATION_URL)}`;
    window.open(url, '_blank');
  });
}
...
function updateCalendarLinks() {
  ...
  const googleUrl =
    ...
    '&dates=' + toGoogleDate(EVENT_START) + '/' + toGoogleDate(EVENT_END) +
  ...
  const outlookUrl =
    ...
    '&body=' + encodeURIComponent(EVENT_DESCRIPTION) +
    '&startdt=' + encodeURIComponent(EVENT_START) +
    '&enddt=' + encodeURIComponent(EVENT_END) +
    '&location=' + encodeURIComponent(EVENT_LOCATION);
...
if (rsvpBtn) {
  rsvpBtn.addEventListener('click', function () {
    ...
    const comentarios = document.getElementById('rsvp-comentarios').value.trim();
    const errorDiv = document.getElementById('rsvp-error');
    let errorMsg = '';
    if (!nombre) errorMsg = 'Por favor, ingresa tu nombre.';
    else if (!cantidad || cantidad < 1) errorMsg = 'Indica cuántas personas confirman.';
    ...
    if (comentarios) mensaje += ` Comentarios: ${comentarios}`;
    const url = `https://wa.me/${RSVP_WHATSAPP_NUMBER}?text=${encodeURIComponent(mensaje)}`;
    window.open(url, '_blank');
  });
}
